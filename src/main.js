...
async function verifyPm2Process() {
    ...
    await exec('pm2 list > ./src/frontend/pm2Logs/process-verification.txt', { encoding: 'utf8' }, (err, stdout, stderr) => {
        if (err) {
        ...
        setTimeout(() => {
            ...
            const logPath = "./src/frontend/pm2Logs/process-verification.txt";
...
async function createPm2Process() {
    ...
    exec('cd src && cd backend && pm2 start server.js --name gerenc-insta-local-API && pm2 save', { encoding: 'utf8' }, (err, stdout, stderr) => {
        if (err) {
